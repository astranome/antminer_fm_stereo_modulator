...
/root/fm
```
Она предоставляет интерфейс для настройки переметров передатчика и мониторинга.
...
**Пример интерфейса утилиты:**
![Архитектура проекта](images/ctrl_console.png)

**Клавиши управления:**
*   `1-5` – Включить/выключить соответствующий параметр (TX, STEREO, RDS, MUTE, цикл PREENPHASIS).
...
6.  **По SSH** зайдите на плату (`tx.local`, логин `root`), перейдите в папку `/root` и запустите скрипт с номером канала, например:
    ...
    ```
    Звук будет передаваться по сети через UDP Multicast — IP-адрес сетевой карты не важен. Удачи!
